...
namespace ToDoList.Bll.Services
{
    public class ToDoItemService : IToDoItemService
    {
        ...
        public async Task<long> AddToDoItemAsync(ToDoItemCreateDto toDoItem)
        {
            ...
            covert.UserID = Int64.Parse(userId!);                                              // convert qlinvotgan infolani todoitem entityga userId ni bervorvommiz
        ...
        public async Task UpdateToDoItemAsync(ToDoItemUpdateDto newItem)
        {
            ...
            var userId = _httpContextAccessor.HttpContext?.User?.FindFirst("UserId")?.Value;   // JWT tokendan userId ni oldik
            convert.UserID = Int64.Parse(userId!);                                              // convert qlinvotgan infolani todoitem entityga userId ni bervorvommiz
